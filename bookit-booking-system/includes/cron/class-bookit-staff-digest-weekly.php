<?php
/**
 * Staff Weekly Digest Cron Job
 *
 * Drains the staff notification digest queue for staff who opted into weekly digests.
 *
...
class Bookit_Staff_Digest_Weekly {
	...
	public static function init(): void {
		...
		// Ensure the "weekly" schedule exists across WP versions/environments.
		add_filter( 'cron_schedules', array( __CLASS__, 'maybe_add_weekly_schedule' ) );
	}
	...
	public static function run_digest_with_tracking(): void {
		...
		update_option(
			'bookit_staff_digest_weekly_last_run',
			gmdate( 'Y-m-d H:i:s' ),
			false
		);

		update_option(
			'bookit_staff_digest_weekly_last_count',
			$count,
			false
		);
	}
	...
	public static function run_digest(): int {
		...
		foreach ( $staff_ids as $staff_id_raw ) {
			...
			$subject   = sprintf( 'Weekly digest: %d booking update(s) for the week', $total_items );
			$html_body = self::build_digest_body( $grouped, $total_items, 'weekly' );
	...
	public static function maybe_add_weekly_schedule( array $schedules ): array {
